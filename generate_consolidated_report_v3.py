...
for loc, vals in loc_data.items():
    ...
    consolidated_locs.append({"Localidad": loc, "Doses": vals["Doses"], "Population": vals["Population"], "Reach": reach})
...
pop_map = {
    "1 año": pop_conapo["1 year"], "2-5 años": pop_conapo["2-5 years"], "6 años": pop_conapo["6 years"],
    "7-9 años": pop_conapo["7-9 years"], "10-19 años": pop_conapo["10-19 years"],
    "20-29 años": pop_conapo["20-29 years"], "30-39 años": pop_conapo["30-39 years"],
...
doc.add_paragraph("Resumen consolidado por nombre de localidad según Formato Concentrado.")
table = doc.add_table(rows=1, cols=4)
table.style = 'Table Grid'
hdr = table.rows[0].cells
hdr[0].text = 'Localidad'; hdr[1].text = 'Dosis Aplicadas'; hdr[2].text = 'Población (INEGI)'; hdr[3].text = 'Alcance (%)'
for item in consolidated_locs:
    row = table.add_row().cells
    row[0].text = item['Localidad']; row[1].text = f"{int(item['Doses']):,}"; row[2].text = f"{int(item['Population']):,}"; row[3].text = f"{item['Reach']:.2f}%"
...
hdr[0].text = 'Grupo de Edad'; hdr[1].text = 'Población (CONAPO)'; hdr[2].text = 'Dosis 2025'; hdr[3].text = 'Dosis 2026'; hdr[4].text = 'Total 25-26'; hdr[5].text = 'Cobertura (%)'

total_pop = 0; total_25 = 0; total_26 = 0; total_acc = 0

for label in age_results.keys():
    pop = pop_map[label]; d_info = age_results[label]; cov = (d_info["Total"] / pop * 100) if pop > 0 else 0
    row = table_age.add_row().cells
    row[0].text = label; row[1].text = f"{int(pop):,}"; row[2].text = f"{int(d_info['2025']):,}"; row[3].text = f"{int(d_info['2026']):,}"; row[4].text = f"{int(d_info['Total']):,}"; row[5].text = f"{cov:.2f}%"
    total_pop += pop; total_25 += d_info["2025"]; total_26 += d_info["2026"]; total_acc += d_info["Total"]

# GLOBAL ROW
global_cov = (total_acc / total_pop * 100) if total_pop > 0 else 0
row = table_age.add_row().cells
row[0].text = 'TOTAL MEZQUITAL'; row[1].text = f"{int(total_pop):,}"; row[2].text = f"{int(total_25):,}"; row[3].text = f"{int(total_26):,}"; row[4].text = f"{int(total_acc):,}"; row[5].text = f"{global_cov:.2f}%"
# Format global row as bold
for cell in row:
