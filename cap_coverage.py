...
try:
    ...
    for entry in data_rows:
        loc = entry["Loc"]; ages = entry["Ages"]; total = entry["Total"]
        if total > 0 and sum(ages) == 0:
            ages = (weights * total).round(0).astype(int).tolist()
            ages[-1] += (total - sum(ages))
        if loc not in final_loc_data: final_loc_data[loc] = {"Ages": np.zeros(len(age_cols)), "Total": 0}
        final_loc_data[loc]["Ages"] += np.array(ages); final_loc_data[loc]["Total"] += total
    ...
    for loc, data in final_loc_data.items():
        ...
        # Cap at 100% (1.0)
        raw_cov = (data["Total"] / pop) if pop > 0 else 0
        cov = min(1.0, raw_cov)
    ...
    # Cap global coverage too? User said "donde se rebase 100%", which includes global.
    ...
    with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
        ...
        for c, v in enumerate(df_final.columns):
            worksheet.write(0, c, v, header_f); worksheet.set_column(c, c, 15)
            if v == 'COBERTURA (%)':
